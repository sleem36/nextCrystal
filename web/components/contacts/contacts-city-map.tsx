  ...
  return (
    <div className="grid min-h-0 gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:items-stretch lg:gap-8">
      ...
      <div className="flex h-full min-h-0 min-w-0 flex-col">
        <div className="flex min-h-0 flex-1 flex-col gap-4 rounded-xl bg-[color:var(--surface-card)] p-4 shadow-[0_1px_3px_rgba(0,0,0,0.06)] ring-1 ring-[color:var(--border-soft)] lg:p-5">
          <h3 className="text-lg font-bold text-[color:var(--text-strong)]">Контактная информация</h3>

          <div className="overflow-hidden rounded-[var(--radius-card)]">
            <ContactBranchGallery key={branchId} images={branch.images} cityLabel={branch.city} />
          </div>

          <div className="space-y-4 border-t border-[color:var(--border-soft)] pt-1 text-sm">
            <div>
              <p className="text-base font-bold leading-snug text-[color:var(--color-link)]">{branch.address}</p>
            </div>

            <div>
              <p className="font-bold text-[color:var(--text-strong)]">Время работы</p>
              <p className="mt-1 leading-relaxed text-[color:var(--text-default)]">{branch.hours}</p>
            </div>

            <div>
              <p className="font-bold text-[color:var(--text-strong)]">Телефон</p>
              <a
            ...
            <div>
              <p className="font-bold text-[color:var(--text-strong)]">Почта</p>
              <a
            ...
            <div>
              <p className="font-bold text-[color:var(--text-strong)]">Юридический отдел</p>
              <a
